...
def main():
    st.set_page_config(layout="wide", page_title="Accumulated Data Keyboard (Range Monitor)")
    ...
    st.title("キーボード座標レンジ確認用ツール")
    st.caption("「Start」ボタンを押すと移動が始まります。**入力する必要はありません。** 画面右上のモニターで最大値・最小値を確認してください。「CSVをダウンロード」でモニターの値を保存できます。")
    ...
    <body>
        <div id="debug-monitor">
            ...
            <div style="margin-top:5px;">Min X: -</div>
            <div style="color:yellow;">Max X: -</div>
            <div style="margin-top:5px;">Min Y: -</div>
            <div style="color:yellow;">Max Y: -</div>
        </div>
        ...
        <script>
            ...
            function updateMonitor() {{
                ...
                // 動作中（Start後）のみ数値を更新する
                if (moveWrap.classList.contains('active')) {{
                    ...
                    debugMonitor.innerHTML = `
                        <div style="font-weight:bold; color:white;">MONITORING...</div>
                        <div style="font-size:12px; color:#ccc;">(Current: ${{currentX.toFixed(1)}}, ${{currentY.toFixed(1)}})</div>
                        <hr style="border-color:#555; margin:5px 0;">
                        <div>Min X: ${{minX.toFixed(1)}}</div>
                        <div style="color:yellow; font-weight:bold;">Max X: ${{maxX.toFixed(1)}}</div>
                        <div style="margin-top:5px;">Min Y: ${{minY.toFixed(1)}}</div>
                        <div style="color:yellow; font-weight:bold;">Max Y: ${{maxY.toFixed(1)}}</div>
                    `;
                }} else {{
                     // 待機中
                     debugMonitor.innerHTML = `
                        <div style="color:white;">STATUS: WAITING...</div>
                        <div style="font-size:12px;">Press Start to track</div>
                        <hr style="border-color:#555; margin:5px 0;">
                        <div>Min X: ${{minX === Infinity ? '-' : minX.toFixed(1)}}</div>
                        <div style="color:yellow;">Max X: ${{maxX === -Infinity ? '-' : maxX.toFixed(1)}}</div>
                        <div>Min Y: ${{minY === Infinity ? '-' : minY.toFixed(1)}}</div>
                        <div style="color:yellow;">Max Y: ${{maxY === -Infinity ? '-' : maxY.toFixed(1)}}</div>
                     `;
                }}
            ...
            function downloadCSV() {{
                // 入力データではなく、モニターの最大値・最小値をCSVとしてダウンロードする
                if (minX === Infinity || maxX === -Infinity || minY === Infinity || maxY === -Infinity) {{
                    alert("まだ測定データがありません。Startボタンを押してしばらくお待ちください。");
                    return;
                }}
                ...
                csvRows.push(`Min_X,${{minX.toFixed(1)}}`);
                csvRows.push(`Max_X,${{maxX.toFixed(1)}}`);
                csvRows.push(`Min_Y,${{minY.toFixed(1)}}`);
                csvRows.push(`Max_Y,${{maxY.toFixed(1)}}`);
                ...
                a.href = url;
                a.download = "keyboard_range_monitor.csv";
